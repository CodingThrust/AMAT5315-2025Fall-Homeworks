...
Tips:
- Degeneracy of zero eigenvalues of $D * I - \text{ajacency matrix}\;(D \geq \text{largest degree})$ $\leftrightarrow$ #Connected components
...
Tips:
- Understand Lanczos: For matrix $A$, find orthonormal basis $Q$ which spans Krylov space $v_0, A v_0, A^2 v_0, \dots$ and calculate $T=Q^\dagger A Q$ which is assumed to have a similar spectrum with $A$.
   Kind of like Gram-Schmidt process in constructing $Q$ but with Hermitian $A$ we need only recurrence relations for three elements and hence $T$ is tridiagonal.
- Understand restarting: Construct small $Q$ and $T$. By the assumption that $Q v$ where $v$ is the dominant eigenvector of $T$ is close to dominant eigenvector of $Q$. Thus $Q v$ is a good initial point for Lanczos.
- Decoupling: Implement Lanczos tridiagonalization and restarting process as two functions
- Care about normalization and orthogonalization
---
